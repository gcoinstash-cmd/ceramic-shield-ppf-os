...
export const AdminPortalModal: React.FC<AdminPortalModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (passcode.toLowerCase() === 'ceramic2026') {
      ...
      <div className="relative w-full max-w-4xl bg-[#121214] border border-rose-500/30 rounded-2xl p-6 sm:p-8 shadow-2xl text-zinc-100 max-h-[90vh] overflow-y-auto">
        ...
        {!isAuthenticated ? (
          <div className="py-8 max-w-md mx-auto text-center">
            ...
            <h3 className="text-2xl font-bold tracking-tight mb-2">CERAMIC SHIELD & PPF OS Portal</h3>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <input
                  ...
                  placeholder="Enter manager passkey (ceramic2026)"
              ...
              {error && (
                <p className="text-red-400 text-xs font-mono">Invalid passkey. Use: ceramic2026</p>
              )}
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-800 pb-6 mb-6">
              <div>
                ...
                <h3 className="text-xl font-bold text-white">CERAMIC SHIELD & PPF OS // BACK-OFFICE</h3>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-zinc-900/60 border border-zinc-800">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      ...
                      <div key="paint_inspections" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>paint_inspections</span>
                      </div>
                    
                      <div key="coating_packages" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>coating_packages</span>
                      </div>
                    
                      <div key="warranty_registry" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>warranty_registry</span>
                      </div>
                    
                      <div key="cure_telemetry" className="p-2.5 rounded bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-300 flex items-center gap-1.5">
                        <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                        <span>cure_telemetry</span>
                      </div>
            ...
            {activeTab === 'telemetry' && (
              <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div key="ACTIVE CURE BAYS" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-500 font-mono">ACTIVE CURE BAYS</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"6 BAYS"}</p>
                    </div>
                  
                    <div key="10-YR WARRANTIES ISSUED" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-500 font-mono">10-YR WARRANTIES ISSUED</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"1,420 UNITS"}</p>
                    </div>
                  
                    <div key="AVERAGE MICRON CORRECTION" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-500 font-mono">AVERAGE MICRON CORRECTION</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"3.2 µm"}</p>
                    </div>
                  
                    <div key="IR CURE COMPLIANCE" className="p-3 rounded-lg bg-zinc-950 border border-zinc-800">
                      <span className="text-xs text-zinc-500 font-mono">IR CURE COMPLIANCE</span>
                      <p className="text-lg font-bold text-rose-400 font-mono mt-0.5">{"100% PASS"}</p>
                    </div>
            ...
            {activeTab === 'sql' && (
              <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 font-mono text-xs text-zinc-300 overflow-x-auto">
                ...
                <div className="text-rose-400">ALTER TABLE paint_inspections ENABLE ROW LEVEL SECURITY;</div>
                <div className="text-zinc-400 mt-1">CREATE POLICY "Allow authenticated read" ON paint_inspections FOR SELECT USING (true);</div>
